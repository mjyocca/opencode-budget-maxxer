...
export async function openGoDashboard(): Promise<{ opened: boolean; url: string; error?: string }> {
  const url = "https://opencode.ai/go";
...
export async function validateGoCredentials(
  workspaceId: string,
  authCookie: string,
  timeoutMs = 8000,
  ...
  try {
    ...
    for (const name of ["rollingUsage", "weeklyUsage", "monthlyUsage"]) {
      if (html.includes(name)) {
        windows.push(name.replace("Usage", ""));
      }
    }

    if (windows.length === 0) {
      return { valid: false, windows: [], error: "Dashboard loaded but no quota data found — page format may have changed." };
    }
